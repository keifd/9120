/*a) key constraints*/
ALTER TABLE lecture ADD CONSTRAINT classroomconflict UNIQUE(classroomid, semester, year, classtime)
INSERT INTO Lecture  VALUES ('abc', 'S1', 2005, 'Wed09', 'EALT'   );
INSERT INTO Lecture  VALUES ('cde', 'S1', 2005, 'Wed09', 'EALT'   );

/*b) referential cosntraints*/
ALTER TABLE WhenOffered ADD CONSTRAINT uoscoderef FOREIGN KEY uoscode REFERENCES UnitOfStudy ON DELETE CASCADE;

/*c) check constraints*/
ALTER TABLE unitofstudy ADD CONSTRAINT max_creduts CHECK (credits <= 4);
ALTER TABLE Transcript ADD CONSTRAINT valid_grades CHECK (grade IN (F’,’P’,’CR’,’D’,’HD’,’W’));
ALTER TABLE WhenOffered ADD CONSTRAINT valid_offer CHECK (semesster IN ('S1','S2','WS','SS'))

/*d) assertions*/
CREATE ASSERTION enrollment_assertion
CHECK (
	NOT EXISTS (
		SELECT 1
		FROM uosoffering
		WHERE enrollment <> (
			SELECT COUNT(*)
			from Transcript
			WHERE uosoffering uoscode = transcript uoscode
			AND uosoffering semester = transcript semester
			AND uosoffering year = transcript year
		)
	)
);
/*2. The room assigned to a unit-of-study must have at least as many seats as the maximum
allowed enrolment for the unit*/
CREATE ASSERTION max_seats
CHECK (
	NOT EXISTS(
		SELECT 1
		FROM uosoffering
		WHERE maxEnrollment > (
			SELECT seats
			FROM lecture JOIN classroom ON lecture.uoscode = classroom.uoscode
			WHERE uosoffering.uoscode = classroom.uoscode
		)
	)
)
/*3. A student cannot be registered for more than 24 credit points in a given semester;*/
CREATE ASSERTION max_credits
check (
	NOT EXISTS(
		SELECT 1
		FROM (transcript NATURAL JOIN
		unitofstudy) NATURAL JOIN student
...
/*e) triggers
update the enrollment number when a student is added in transcript*/
CREATE OR REPLACE FUNCTION updateEnrol() RETURNS trigger AS $$
BEGIN
	UPDATE uosoffering u
	SET u.enrollment = u.enrollment + 1
	WHERE u.uoscode = NEW.uoscode AND u.semester = NEW.semester
	AND u.year = NEW.year;
	RETURN NEW;
...
/*Need a AFTER DELETE if a student drops a class, 
need a BEFORE UPDATE trigger to ensure maxenrollment can't be changed to less than current enrollment*/
...
CREATE TABLE Requires (
  ...
  constraint Requires_uoSCode_fk_UOS FOREIGN KEY (uoSCode) REFERENCES UnitOfStudy(uoSCode),
...
INSERT INTO Student VALUES (309187546, 'Sally Waters', 'Robot', 'Kingsford');
...
INSERT INTO Transcript VALUES (307088592,'INFO2120', 'S1', 2009,'F');
INSERT INTO Transcript VALUES (307088592,'INFO2120', 'S1', 2010,'P');
